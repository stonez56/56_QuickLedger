...
export const SystemSettings: React.FC<SystemSettingsProps> = ({ config, records }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {message && (
        <div className={`p-4 rounded-xl flex items-start ${message.type === 'error' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-bold text-slate-100">系統安全性與權限</h2>
          </div>
          
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">API Secret (登入密碼)</label>
              <input
                type="text"
                value={secret}
                onChange={e => setSecret(e.target.value)}
                className="w-full bg-slate-900 border border-slate-700 text-slate-100 rounded-lg py-2 px-3 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                placeholder="輸入新的登入密碼"
              />
            </div>
            
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">允許登入的 Google 信箱 (以逗號分隔)</label>
              <textarea
                value={usersStr}
              ...
              />
              <p className="text-xs text-slate-500 mt-2">只有列在此處的信箱，在通過 Secret 驗證後才能寫入資料。</p>
            </div>

            <button
              onClick={handleSaveSecurity}
              disabled={loading}
              className="w-full flex items-center justify-center py-2.5 px-4 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg font-medium transition-colors disabled:opacity-50"
            >
        ...
        <Card className="p-6 h-fit">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-bold text-slate-100">本機備份 (Local Backup)</h2>
          </div>
          ...
          <button
             ...
             className="w-full flex items-center justify-center py-2.5 px-4 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-sky-400 rounded-lg font-medium transition-colors"
          >
      ...
      <Card className="p-6">
         <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
               ...
               <h2 className="text-xl font-bold text-slate-100">雲端備份與還原 (Google Drive)</h2>
            </div>
         ...
         <p className="text-sm text-slate-400 mb-6 leading-relaxed bg-slate-900/50 p-4 rounded-lg border border-slate-800">
         ...
         <div className="mt-8 border-t border-slate-800 pt-6">
            <div className="flex items-center justify-between mb-4">
               ...
               <button onClick={fetchBackups} className="text-sm text-slate-400 hover:text-slate-200 transition-colors flex items-center">
            ...
            {loadingBackups ? (
               <div className="py-8 text-center text-slate-500">讀取中...</div>
            ) : backups.length === 0 ? (
               <div className="py-8 text-center text-slate-500 bg-slate-900/30 rounded-lg border border-dashed border-slate-800">
               ...
               <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                 {backups.map(b => (
                    <div key={b.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-900/50 hover:bg-slate-800/80 border border-slate-800 rounded-lg transition-colors gap-4">
                       <div className="flex items-start gap-3 overflow-hidden">
                          <FileClock className="w-5 h-5 text-slate-500 shrink-0 mt-0.5" />
                          <div className="min-w-0">
                             <p className="font-medium text-slate-200 truncate" title={b.name}>{b.name}</p>
                             <p className="text-xs text-slate-500 mt-1">建立時間: {b.dateCreated}</p>
                          </div>
                       </div>
                       <button
