...
3.  **執行初始化與備份設定**:
    ...
    * ⚠️ **[重要] 強制觸發 Google Drive 雲端硬碟授權**：若執行 `setup` 時尚未跳出請求存取雲端硬碟的視窗，請在上方下拉選單改選為 **`backupData`** 函式，並點擊一次執行 (Run)。此動作將強制 Google 進行權限審查並彈出授權視窗。授權完成後，即便下方顯示 `e` 參數未定義的紅色錯誤也是完全正常的，直接忽略即可。
...
## 🧙‍♂️ Developer Notes: AI Magic & Architecture Tips

為確保未來維護者與使用者了解系統背後的「隱藏魔法」，特此記錄核心架構細則：

1. **AI 稅務顧問 (Tax Advisor) 自動分類**：
   - 當使用者在「備註」或「會計科目」欄位輸入或被掃描出 `油資`、`加油`、`洗車`、`停車`、`職工福利`、`交際費`、`餐飲`、`保險費` 等關鍵字時：
   - 系統將**自動**把稅額 (Tax) 歸零。
   - 自動將稅代碼改為 **4 - 不計稅/不可扣抵**。
   - 自動於備註前墜加上提示詞（如：`[公務車號 ABC-1234]`）。
2. **軟刪除架構 (Soft-Delete Architecture)**：
   - **請勿真的刪除已經有記帳歷史的會計科目**，這會導致舊有資料於編輯時遺失映射。
   - 若您不再使用某個科目，請在 Google Sheet (Config) 中將名稱前加上 `(停用)`。例如 `(停用) 舊交際費`。
   - **實作邏輯**：系統會主動過濾並**隱藏**所有包含 `(停用)` 字首的項目，使用者在「新增憑證」時再也不會看見。但若您「編輯」一筆古代歷史訂單，該隱藏項目會幽靈般暫時浮出供編輯視窗維持狀態，達到完美相容。
3. **無痛取消編輯與快取還原 (React State & Viewport Targeting)**：
   - 「歷史查詢」功能 (`SearchPanel`) 在進入編輯視圖時，切勿使用 `unmount` `(activeTab === 'search' && <SearchPanel/>)`，而是透過 CSS `className="hidden"` 隱形。這能保證使用者打到一半的關鍵字與從 Google Sheet 撈回的百筆資料**完美快取保留**。
   - 系統為「返回滾動點」刻意實作了分離機制：`lastEditedId` (給予藍色發光與已更新徽章，僅限存檔) / `scrollTargetId` (單純畫面滾動，退回原位使用)。確保使用者點擊「取消編輯」時能閃電無縫退回原位。
4. **收據免統編強驗證 (Format 99 Bypass)**：
   - 憑證防呆機制在遇到「格式代號 `99` (收據/無發票)」時，會特例放行。允許使用者輸入非 8 碼的統編或直接留白。請務必保留此業務邏輯，切勿將統編驗證改為全局強制 8 碼。
5. **零延遲儀表板快取 (Dashboard Caching)**：
...
7. **加上 帳務歸屬-備註說明**
   - 下拉選單選項：無墊付、業主墊付、員工墊付 
   - 當有業主/員工務付金額時,快速記錄
   - 預設值：無墊付
8. **Dashboard 快取失效實作計畫**
   - 我們已經實作了一套穩健的快取失效策略，以確保儀表板統計數據能夠反映最新資料。  
   - 只要有紀錄或相關設定被修改，系統就會清除 `sessionStorage` 的快取。
---
